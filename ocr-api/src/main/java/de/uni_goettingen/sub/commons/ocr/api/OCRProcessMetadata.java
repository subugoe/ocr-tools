 ...
 * {@link OCRProcess} and it's results
 * This can be used to filter the results for accuracy or to save it for further processing.
 */
public interface OCRProcessMetadata {
	...
	/**
	 * Gets a string representation of the line break used in the results.
	 * One of "CR, "CR/LF" or "LF".
	 *
	...
	/**
	 * Gets the format of the encoded result, consider using a controled vocabulary
	 * like this "SGML", "XML", "HTML", "TXT" and "XHTML". Note that implementations can
	 * return their own values since this is just a String.
	 *
	 * @return the format
	 */
	...
	/**
	 * Gets the document type. This is valid for "SGML", "XML", "HTML" and "XHTML"
	 * and should reference a DTD or schema, if possible by using a URI.
	 *
	...
	/**
	 * Gets processing note for the {OCRProcess}. This can be used to add a general
	 * note to the processing / recognition process. Implementors may choose this
	 * to encode additional machine readable data as escaped XML. 
	 *
	 ...
	 * Implementations should try to convert this into a percentage value. 
	 * If the engine isn't able to report the confidence level based on a process an
	 * {@link java.lang.UnsupportedOperationException} should be thrown.
	 *
	...
	/**
	 * Gets the duration of a process.this returns 0 if the process hasn't started yet.
	 * The timing might be inaccurate if the process failed.
